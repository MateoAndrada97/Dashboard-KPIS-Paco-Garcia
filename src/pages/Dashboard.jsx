...
export default function Dashboard() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return baseData.uniqueOrders.filter((order) => {
      ...
      const courrierOk =
        filters.courrier === "all" || order.courrier === filters.courrier;
      const paymentOk =
  ...
  const metrics = useMemo(() => {
    if (!baseData) {
      const totalRevenue = mockData.salesDaily.reduce(
        (acc, item) => acc + item.revenue,
        0
      );
      const totalOrders = mockData.salesDaily.reduce(
          ...
          <div className="mx-auto max-w-7xl">
            <header className="pg-hero pg-pattern rounded-[32px] px-8 py-8 shadow-xl shadow-[rgba(0,72,59,0.12)]">
              ...
              <p className="mt-3 max-w-3xl text-base text-white/78">
                Filtros por estado, fecha, ciudad, provincia, courrier y medio
                de pago.
              </p>
            </header>
              ...
              <div className="pg-card pg-pattern-soft rounded-3xl p-5">
                ...
                <div className="mt-6 space-y-4">
                  <div className="rounded-2xl bg-[var(--pg-light)] p-4">
                    <p className="text-sm text-[var(--pg-deep)]/70">
                      Producto líder
                    </p>
            ...
            <section className="mt-6 grid gap-6 xl:grid-cols-2">
              <ProductsTable
                ...
                title={`Top 10 del mes actual (${formatMonthKey(
                  metrics.currentMonthKey
                )})`}
              />
              <ProductsTable
                products={metrics.topProductsPreviousMonth || []}
                title={`Top 10 del mes anterior (${formatMonthKey(
                  metrics.previousMonthKey
                )})`}
              />
            </section>
          </div>
